...
# totp = TOTP("7ANZBHT7FGTTOFIWXFHT7HCETSJTK3XM")
# token = totp.now()
# print(token)



cwd = os.chdir("C:\\Users\\sugho\\Jupyter Notebooks\\Electronic Stock Exchange")

def autologin():
    token_path = "api_key.txt"
    key_secret = open(token_path,'r').read().split()
    kite = KiteConnect(api_key=key_secret[0])
    service = webdriver.chrome.service.Service('./msedgedriver')
    service.start()
    ...
    totp = driver.find_element("xpath",'/html/body/div[1]/div/div[2]/div[1]/div/div/div[2]/form/div[2]/input')
